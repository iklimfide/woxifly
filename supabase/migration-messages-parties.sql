-- Mesajlara gönderici + alıcı snapshot alanları ve sütun sırası
-- Supabase Dashboard > SQL Editor'da bir kez çalıştırın.
--
-- DM: receiver_id / receiver_username = karşı taraf
-- Grup: receiver_id = null, receiver_username = "{İlçe} Genel Odası"
...
comment on column public.messages.sender_username is
  'Gönderim anındaki gönderici rumuzu.';
comment on column public.messages.receiver_id is
  'DM: alıcı kullanıcı. Grup mesajlarında null.';
comment on column public.messages.receiver_username is
  'DM: alıcı rumuzu. Grup: "{ilçe} Genel Odası".';
...
drop function if exists public.set_message_sender_username();
...
-- message_reactions RLS politikaları messages tablosuna bağlı; swap öncesi kaldır
...
-- message_reactions politikalarını yeniden kur
drop policy if exists "reactions: read dm if member" on public.message_reactions;
